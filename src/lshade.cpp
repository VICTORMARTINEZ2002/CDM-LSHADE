...
double LSHADE::run(){
  ...
  for(int i=0; i < arc_size; i++) archive.push_back((double*)malloc(sizeof(double) * problem_size));
...
void LSHADE::operateCurrentToPBest1BinWithArchive(const vector<double*> &pop, double* child, int &target, int &p_best_individual, double &scaling_factor, double &cross_rate, const vector<double*> &archive, int &arc_ind_count){  
	int r1,r2;
	do{
		r1 = rand()%pop_size;
	}while(r1==target);
